...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.xjbed.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0`;
...
async function run() {
  try {
    ...
    // Route to get all applied instructors
    app.get("/applied-instructors", async (req, res) => {
      const result = await appliedCollection.find().toArray(); // Get all documents in the collection
      res.send(result);
    });
